...
class TimeBudgetRecorder():
    ...
    def __init__(self, quiet_mode: bool = False, active: bool = False):
    ...
    def start(self, block_name: str):
        if block_name in self.start_times:
            ...
            warnings.warn(
                f"timebudget is confused: timebudget.start({block_name}) without end")
    ...
    def end(self, block_name: str, quiet: Optional[bool] = None) -> float:
        ...
        if block_name not in self.start_times:
            warnings.warn(
                f"timebudget is confused: timebudget.end({block_name}) without start")
            return float('NaN')
...
def annotate(func: Callable, quiet: Optional[bool]):
    ...
    def inner(*args, **kwargs):
        cl_name = ''
        # check if first function argument is 'self', i.e. a class instance
        if len(args):
...
def annotate_or_with_block(func_or_name: Union[Callable, str], quiet: Optional[bool] = None):
    ...
    raise RuntimeError(
        "timebudget: Don't know what to do. Either @annotate or with:block")
...
def set_active(active: bool = True):
    """Activate or deactivate the tracking with timebudget.
    """
    _default_recorder.is_active = active
...
timebudget.set_active = set_active
